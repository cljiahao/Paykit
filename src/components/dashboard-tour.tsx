 ...
 * driver.js overlay. Auto-runs once on the transaction-count stat for
 * vendors who haven't seen it (server-tracked via `seen`); the button
 * replays it anytime.
 */
export function DashboardTour({ seen }: { seen: boolean }) {
  ...
  // land back on the dashboard page (step 1's anchor).
  ...
  // Auto-run once on first visit, only on the dashboard page (step 1's anchor).
  useEffect(() => {
  ...
  // Run a cross-page replay once we've actually arrived on the dashboard
  // page — waits for the route + the anchor instead of guessing with a
  // fixed delay.
  useEffect(() => {
  ...
  function onReplay() {
    if (pathname !== "/dashboard") {
      // Step 1 spotlights the transaction-count stat, so get there first, then run.
  ...
  return (
    <button
      ...
      className="fixed right-5 bottom-5 z-40 flex h-12 w-12 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-lg ring-1 ring-black/5 transition-transform hover:scale-105"
    >
