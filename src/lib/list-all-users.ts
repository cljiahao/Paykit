 ...
 * missing from the admin console's email lookup. Stops at the first partial
 * page (fewer than perPage results), which per the GoTrue admin API always
 * means the last page. Mirrors the {data, error} shape of a single
 * listUsers() call so callers' existing error-checking pattern doesn't need
 * to change. Ported verbatim from loopkit's src/lib/list-all-users.ts.
 */
