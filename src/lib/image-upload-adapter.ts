...
// from the `pathPrefix` prop each call site passes (the vendor id), so
// there's no vendor-scoped work left to do here: this just writes the blob
// and resolves the public URL, the exact call paykit's old local
...
export const uploadPaykitImage: ImageUploaderProps["onUpload"] = async ({
  bucket,
  path,
  blob,
  contentType,
