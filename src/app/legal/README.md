...
- `accept/actions.ts` — `acceptLegalTerms` server action. Re-checks for a
  ...
  bearer-authed with `MERQO_CUSTOMER_SECRET`, `kit_slug: "paykit"`, each body
  carrying the SHA-256 of that doc's `getLegalDocSource(...)` plus
  `legal_name` and the vendor's real `ip`/`user_agent` (read via
  `headers()`/`clientIp` from `@/lib/rate-limit`, since this action runs on
  the vendor's own browser submission). Each call is independent, and merqo
  maps a duplicate `(email, doc_type, doc_version)` to a success, so a
  conflict on one doc never blocks the other. On success it primes the local
